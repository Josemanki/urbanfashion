  ...
  return (
    <>
      <div className="p-6 flex flex-wrap justify-between gap-4">
        {cat
          ? filteredProducts.map((productProps) => <Product {...productProps} key={productProps.id} />)
          : products.slice(0, 8).map((productProps) => <Product {...productProps} key={productProps.id} />)}
      </div>
    </>
  );
};
